#!/usr/bin/env python3
"""
CHIMERA v9.2 - Object-Centric Reasoning Brain

Extends v9.1 by adding a robust object detection module and refactoring
the solver to reason about objects and their transformations, not just pixels.
...
class LivingBrainV9_2:
    ...
    def __init__(self, memory_size=(256, 256)):
        ...
        print("="*80)
        print("CHIMERA v9.2 - OBJECT DETECTION BRAIN")
        print("="*80)
        ...
        print(f"[BRAIN] v9.2 awakened and ready")
        print("="*80)
    ...
    def _detect_objects(self, grid: np.ndarray) -> List[ArcObject]:
        """Detects objects in a grid using connected components.
        Ignores the most common color (usually background)."""
        if grid.size == 0:
            return []
        ...
        for color in colors:
            ...
            for i in range(1, num_features + 1):
                ...
                obj_grid = grid[y_min:y_max+1, x_min:x_max+1]
                obj_grid_masked = np.where(obj_mask[y_min:y_max+1, x_min:x_max+1], obj_grid, -1)

                objects.append(ArcObject(grid=obj_grid_masked, color=color, position=(y_min, x_min)))
        return objects

    def solve_task(self, task: Dict, verbose: bool = True) -> List[List[List[List[int]]]]:
        self.tasks_processed += 1
        if verbose: print(f"\n[v9.2] Task #{self.tasks_processed} | Age: {time.time() - self.birth_time:.1f}s")
        ...
        try:
            ...
            for ex in task['train']:
                ...
                in_objects = self._detect_objects(in_grid)
                out_objects = self._detect_objects(out_grid)
                train_pairs.append({'input': in_objects, 'output': out_objects, 'out_grid': out_grid})
        ...
        for transform_name in self._generate_spatial_transforms(np.zeros((1,1))):
            ...
            for pair in train_pairs:
                ...
                reconstructed_grid = np.zeros_like(pair['out_grid'])
                for in_obj in pair['input']:
                    ...
                    patch = reconstructed_grid[y:y+h, x:x+w]
                    patch[transformed_obj_grid != -1] = transformed_obj_grid[transformed_obj_grid != -1]
        ...
        for test_case in task['test']:
            ...
            final_grid = np.zeros_like(test_input_grid)
    ...
    def get_stats(self):
        return {'version': '9.2', 'tasks_processed': self.tasks_processed, 'age_seconds': time.time() - self.birth_time}

_global_brain_v9_2 = None
def get_brain_v9_2():
    global _global_brain_v9_2
    if _global_brain_v9_2 is None: _global_brain_v9_2 = LivingBrainV9_2()
    return _global_brain_v9_2

def solve_arc_task(task: Dict, verbose: bool = True) -> List[List[List[List[int]]]]:
    return get_brain_v9_2().solve_task(task, verbose=verbose)


if __name__ == "__main__":
    print("="*80)
    print("CHIMERA v9.2 - LOCAL TEST")
    print("="*80)
    brain = get_brain_v9_2()
